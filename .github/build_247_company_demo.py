...
.company247 .settingsMark{font-size:0;background:#fff;padding:4px}.company247 .settingsMark:before{content:'24/7';font-size:12px;color:#12385b;font-weight:950}
...
once('<div class="brand">ZONE SKETCH<small>BY WILL FLOOD · PLAN / ZONE MAKER · BETA v0.50</small></div>',
     '<div class="brand companyBrand" aria-label="24/7 Protection company demo"><img class="companyBrandLogo" src="logo.svg" alt="24/7 Protection"><span class="companyBrandTag">COMPANY DEMO</span></div>','top brand')
once('FIRE &amp; SECURITY · FIELD WORKSPACE · BETA','24/7 PROTECTION · FIRE &amp; SECURITY FIELD WORKSPACE · COMPANY DEMO','home eyebrow')
once('<h1>Zone Sketch <span class="byWill">by Will</span></h1>','<div class="companyHeroBrand"><img src="logo.svg" alt="24/7 Protection"><div><strong>24/7 Protection</strong><small>FIRE &amp; SECURITY FIELD WORKSPACE</small></div></div><h1>Zone Sketch <span class="byWill">company edition</span></h1>','home brand')
once('<div class="creatorLine">Designed &amp; built by <strong>Will Flood</strong></div>','<div class="creatorLine">24/7 Protection branded demo · Zone Sketch platform by <strong>Will Flood</strong></div><div class="companyIsolationNote">Separate demo storage · projects and backups created here cannot mix with the normal Zone Sketch app.</div>','home attribution')
...
once('<strong>Site-ready workspace</strong><small>Local autosave · touch-first controls · offline project data</small>','<strong>24/7 Protection workspace demo</strong><small>Local autosave · touch-first controls · isolated company-demo project data</small>','home badge')
once('<small>BY WILL FLOOD · ZONE PLAN + SURVEY → WIRING → AS-FIT</small>','<small>24/7 PROTECTION · ZONE PLAN + SURVEY → WIRING → AS-FIT · COMPANY DEMO</small>','circuit identity')
once('<div class="settingsMark">ZS</div>','<div class="settingsMark">24/7</div>','settings mark')
once('<span>ZONE SKETCH · <b>v0.50</b> · DESIGNED &amp; BUILT BY <b>WILL FLOOD</b></span>','<span><b>24/7 PROTECTION</b> · COMPANY DEMO · ZONE SKETCH v0.50 · PLATFORM BY <b>WILL FLOOD</b> · SEPARATE DEMO STORAGE</span>','settings footer')
...
# HARD SEPARATION: independent DB, independent preferences, independent backup format.
...
once("JSON.stringify({format:'ZoneSketchByWill',version:1,project:state})","JSON.stringify({format:'ZoneSketch247Protection',version:1,project:state})",'separate backup format')
once("if(!['ZoneSketch','ZoneSketchByWill'].includes(raw?.format)||raw.version!==1", "if(raw?.format!=='ZoneSketch247Protection'||raw.version!==1", 'reject normal backups')
once("alert('This is not a supported Zone Sketch backup. Existing surveys have not been changed.');","alert('This file is not a 24/7 Protection demo backup. Nothing has been changed.');",'company backup error')
...
once("x.fillText('Zone Sketch by Will · '+(mode==='survey'?'Site survey':'Zone plan')+' draft · Office verification required',65,2040);","x.fillText('24/7 Protection · '+(mode==='survey'?'Site survey':'Zone plan')+' draft · Powered by Zone Sketch · Office verification required',65,2040);",'pdf footer brand')
...
OUT_DIR.mkdir(parents=True,exist_ok=True);OUT.write_text(t)
manifest={'name':'24/7 Protection Zone Sketch Demo','short_name':'24/7 Protection','id':'/FIRE-ZONE-PLAN-APP/company-demos/247-protection/','start_url':'./','scope':'./','display':'standalone','background_color':'#edf2f7','theme_color':'#10263d','icons':[{'src':'logo.svg','sizes':'any','type':'image/svg+xml','purpose':'any maskable'}]}
...
assert "format:'ZoneSketch247Protection'" in t
assert "['ZoneSketch','ZoneSketchByWill'].includes(raw?.format)" not in t
