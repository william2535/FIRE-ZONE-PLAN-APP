...
(async()=>{
 ...
 try{
  ...
  const visibleStats=await imageStats();assert(visibleStats.white>500,'Zone Plan export should contain a clean white drawing sheet');assert(visibleStats.dark>100,'Building drawing should be present in the clean export with tracing picture visible');
  await page.locator('#togglePicture').click();await page.evaluate(()=>window.exported=null);await page.locator('#shareBtn').click();await page.locator('#exportPng').click();await page.waitForFunction(()=>window.exported?.data);const hiddenStats=await imageStats();assert.equal(hiddenStats.width,visibleStats.width);assert.equal(hiddenStats.height,visibleStats.height);
