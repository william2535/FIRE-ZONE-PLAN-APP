...
const expose=`window.cbBridgeTest={
 ...
 inspect:endAnchor=>{const r=$('cbCanvas').getBoundingClientRect(),stroke=cbEdit?.stroke,pair=stroke&&endAnchor?cbEditResolveAnchors(stroke.start,endAnchor):null;if(!stroke||!pair)return null;const startPx=cbBoardPx(pair.startPoint,r.width,r.height),endPx=cbBoardPx(pair.endPoint,r.width,r.height),d=cbEditEnsureDraft(),path=cbEditCleanStroke(stroke.raw,startPx,endPx,r.width,r.height,d.cleanup),conflict=cbEditCrossingsForPath(path,pair.legIndex,pair.startPos,pair.endPos,r.width,r.height);return JSON.parse(JSON.stringify({pair,path:path.map(p=>cbPxBoard(p,r.width,r.height)),conflict,raw:stroke.raw.map(p=>cbPxBoard(p,r.width,r.height)),cell:cbRouteCellPx(r.width,r.height)}))},
...
(async()=>{await new Promise(ok=>server.once('listening',ok));const browser=await chromium.launch({headless:true});try{
 ...
 const pts=s.draft.legs[0].points,m0=pts[2],m1=pts[3],start={x:m0.x+(m1.x-m0.x)*.20,y:m0.y+(m1.y-m0.y)*.20},end={x:m0.x+(m1.x-m0.x)*.80,y:m0.y+(m1.y-m0.y)*.80},returnY=s.draft.legs[2].points[1].y,routeY=Math.max(.08,Math.min(.92,returnY+(returnY<.78?.10:-.10)));
 const a=await page.evaluate(p=>cbBridgeTest.anchor(p),start),b=await page.evaluate(p=>cbBridgeTest.anchor(p),end);assert(a&&b,'replacement anchors must be hittable');assert.equal(a.kind,'segment','start anchor must stay outside device hit zones');assert.equal(b.kind,'segment','end anchor must stay outside device hit zones');
 const draw=async()=>{await page.evaluate(({a,start})=>cbBridgeTest.start(a,start),{a,start});for(const p of [{x:start.x,y:routeY},{x:end.x,y:routeY}])await page.evaluate(p=>cbBridgeTest.move(p),p);const inspect=await page.evaluate(b=>cbBridgeTest.inspect(b),b),finish=await page.evaluate(b=>cbBridgeTest.finish(b),b);return{inspect,finish}};
 let attempt=await draw();assert.equal(attempt.finish,false,'real geometric crossing must be rejected with Bridge OFF');assert(attempt.inspect?.conflict?.crossings?.length>=1,'Bridge OFF probe must contain a real crossing');s=await page.evaluate(()=>cbBridgeTest.read());assert.equal(s.draft.pending,null,'Bridge OFF rejection must not leave staged replacement');
