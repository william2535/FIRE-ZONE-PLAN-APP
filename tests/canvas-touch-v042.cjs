...
(async()=>{
 ...
 try{
  ...
  await page.locator('#moveModeTop').click();await page.locator('#viewFit').click();await page.locator('#surveyModeBtn').click();await page.waitForFunction(()=>document.querySelector('.app')?.classList.contains('surveyMode')&&document.querySelector('#surveyModeBtn')?.getAttribute('aria-pressed')==='true');await page.waitForTimeout(60);
  const surveyBaseline=(await page.evaluate(()=>window.touchStable.read())).zoom;await pinch(1.7,18,22);r=await page.evaluate(()=>window.touchStable.read());assert(r.zoom>surveyBaseline*1.6,'Survey mode must allow two-finger zoom without a move toggle');assert.equal(r.state.symbols.length,0,'two-finger Survey navigation must not stamp a detector');
