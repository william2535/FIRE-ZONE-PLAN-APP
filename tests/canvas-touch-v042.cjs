...
(async()=>{
 ...
 html=html.replace(marker,marker+`window.touchStable={read:()=>({zoom,state:JSON.parse(JSON.stringify(state)),pan:{...pan}}),start:async()=>{state=fresh();state.site='Touch';state.image=blankImage();state.isBlank=true;ensureUiState();ensureFloors();syncFloor();currentProject='touch';projectsReady=false;document.querySelector('#projectsHome').hidden=true;document.querySelector('.app').inert=false;await setImage(state.image,false);renderFloors();renderZones();syncGrid();updateButtons();draw()}};`);
 ...
 try{
  ...
  await page.locator('#moveModeTop').click();await page.locator('#viewFit').click();await page.locator('#surveyModeBtn').click();
